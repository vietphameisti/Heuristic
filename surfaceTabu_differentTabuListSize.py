...
def generateNeighbor(polygone,rect):
    global xmin,xmax,ymin,ymax
    anglemin = 1
    anglemax = 89
    boolOK = False
...
def bestNeighbor(path, nbNeigh, ltaboo):
    ...
    b = generateNeighbor(polygone,path)
    bestV = b['pos']
    bestDist = b['eval']
    for i in range(nbNeigh):
        Neigh = generateNeighbor(polygone,path)
...
# initialization of the taboo list
    ...
    #display figure at each size of tabu list
    fig = plt.figure()
...
# main loop of the taboo algorithm
    while i <= iterMax:
        ...
        if i % 10 == 0:
            H_energy.append(dist)
            H_time.append(i)
            H_best.append(best_dist)
    ...
    return boxplotData,H_energy,H_best,H_time
# FIN : affichages
...
for i in nb:
    ...
    boxplotData.append(boxplot)
    Henergy.append(Henergy_nb)
    Hbest.append(Hbest_nb)
    Htime.append(Htime_nb)


fig2 = figure()
subplot(1,3,1) # 2 subplot in 1 figure, 1st subplot
semilogy(Htime[0], Henergy[0],'r',label="TabuList size= 1")
semilogy(Htime[0], Henergy[1],'g',label="TabuList size= 50")
semilogy(Htime[0], Henergy[2],'b',label="TabuList size= 100")
legend()
title("Evolution de la solution en cours")
xlabel('time')
ylabel('Energy')
subplot(1,3,2) # 2 nd subplot
semilogy(Htime[0], Hbest[0],'r',label="TabuList size= 1")
semilogy(Htime[0], Hbest[1],'g',label="TabuList size= 50")
semilogy(Htime[0], Hbest[2],'b',label="TabuList size= 100")
legend()
title('Evolution de la meilleure aire')
xlabel('time')
ylabel('aire')

ax=fig2.add_subplot(1,3,3)
bp=ax.boxplot(boxplotData,patch_artist=True)
pyplot.gca().xaxis.set_ticklabels(['Size 1', 'Size 50', 'Size 100'])
for box in bp['boxes']:
...
#ax=fig.add_subplot()
#bp=ax.boxplot(boxplotData,patch_artist=True)
# pyplot.gca().xaxis.set_ticklabels(['TabuList size 1', 'TabuList size 7', 'TabuList size 100'])
# for box in bp['boxes']:
#     # change outline color
